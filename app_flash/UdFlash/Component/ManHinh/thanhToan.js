...
const ThanhToan = ({ route }) => {
  ...
  const getlistgiohang = async () => {
    const url_api_giohang = 'http://10.24.0.248:9997/giohang/' + loginInfo._id;
  ...
  const capNhatSanPham = async (sanPhamId, soLuongMoi, soLuongBan) => {
    const url_api_capnhat = 'http://10.24.0.248:9997/giohang/cap-nhat-sanpham';
    const data = {
      gioHang: [
        {
          sanPhamId: sanPhamId,
  ...
  const Save_UserMua = () => {
    ...
    let url_api_hoadon = 'http://10.24.0.248:9997/hoadon/them/' + loginInfo._id;
  ...
  const DelPro = () => {
    let url_api_del = 'http://10.24.0.248:9997/giohang/xoa/' + loginInfo._id;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ borderTopWidth: 0.5, marginBottom: 5 }}>
        <TouchableOpacity onPress={() => { navigation.navigate('AllDiachi') }} style={{ flexDirection: 'row', backgroundColor:'#F2F2F2'}}>
          <View>
            <View style={{ flexDirection: 'row', padding: 20, }}>
              ...
              <View style={{ flexDirection: 'column', width: 350, height: 50, marginLeft: 8, marginBottom: 5 }}>
                ...
                {item?.name && item?.phone && item?.address ? (
                  ...
                  <Text style={[{ color: '#000' }, styles.placeholderText]}>Mời bạn chọn địa chỉ nhận hàng</Text>
                )}
              </View>
