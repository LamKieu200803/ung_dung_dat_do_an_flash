...
const ChiTietSanPham = ({ route, navigation }) => {
  ...
  const Save_Pro = () => {
    ...
    let url_api_giohang = "http://10.24.0.248:9997/giohang/them/" + loginInfo._id + "/" + idsp;
    let url_api_gio = "http://10.24.0.248:9997/giohang/" + loginInfo._id;
    ...
    fetch(url_api_gio)
      ...
      .then((json) => {
        ...
        if (existingProduct) {
          ...
          return fetch(
            "http://10.24.0.248:9997/giohang/sua/" + loginInfo._id + "/" + idsp,
            {
              method: "PUT",
              headers: {
  ...
  const getBl = async () => {
    let api_bl = 'http://10.24.0.248:9997/binhluan/' + idsp
    try {
